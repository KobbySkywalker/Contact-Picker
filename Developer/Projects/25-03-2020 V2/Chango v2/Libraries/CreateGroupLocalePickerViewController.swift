//
//  CreateGroupPickerViewController.swift
//  Chango v2
//
//  Created by Hosny Savage on 18/08/2021.
...
extension UIAlertController {
    ...
    func addCreateGroupLocalePicker(type: CreateGroupLocalePickerViewController.Kind, selection: @escaping CreateGroupLocalePickerViewController.Selection) {
        var info: CreateGroupLocaleInfo?
        let selection: CreateGroupLocalePickerViewController.Selection = selection
        let buttonSelect: UIAlertAction = UIAlertAction(title: "Select", style: .default) { action in
            selection(info)
        }
        buttonSelect.isEnabled = false
        
        let vc = CreateGroupLocalePickerViewController(type: type) { new in
            info = new
...
final class CreateGroupLocalePickerViewController: UIViewController {
    ...
    public typealias Selection = (CreateGroupLocaleInfo?) -> Swift.Void
    ...
    fileprivate var orderedInfo = [String: [CreateGroupLocaleInfo]]()
    fileprivate var sortedInfoKeys = [String]()
    fileprivate var filteredInfo: [CreateGroupLocaleInfo] = []
    fileprivate var selectedInfo: CreateGroupLocaleInfo?
    ...
    func updateInfo() {
        ...
        CreateGroupLocaleStore.fetch { [unowned self] result in
            switch result {
                
            case .success(let orderedInfo):
                let data: [String: [CreateGroupLocaleInfo]] = orderedInfo
                /*
                 switch self.type {
                 case .currency:
    ...
    func info(at indexPath: IndexPath) -> CreateGroupLocaleInfo? {
...
extension CreateGroupLocalePickerViewController: UISearchResultsUpdating {
...
extension CreateGroupLocalePickerViewController: UISearchBarDelegate {
...
extension CreateGroupLocalePickerViewController: UITableViewDelegate {
...
extension CreateGroupLocalePickerViewController: UITableViewDataSource {
