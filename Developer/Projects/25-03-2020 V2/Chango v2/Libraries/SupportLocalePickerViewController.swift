//
//  SupportLocalePickerViewController.swift
//  Chango v2
//
//  Created by Hosny Savage on 23/08/2020.
...
extension UIAlertController {
    ...
    func addSupportLocalePicker(type: SupportLocalePickerViewController.Kind, selection: @escaping SupportLocalePickerViewController.Selection) {
        var info: SupportLocaleInfo?
        let selection: SupportLocalePickerViewController.Selection = selection
        let buttonSelect: UIAlertAction = UIAlertAction(title: "Select", style: .default) { action in
            selection(info)
        }
        buttonSelect.isEnabled = false
        
        let vc = SupportLocalePickerViewController(type: type) { new in
            info = new
...
final class SupportLocalePickerViewController: UIViewController {
    ...
    public typealias Selection = (SupportLocaleInfo?) -> Swift.Void
    ...
    fileprivate var orderedInfo = [String: [SupportLocaleInfo]]()
    fileprivate var sortedInfoKeys = [String]()
    fileprivate var filteredInfo: [SupportLocaleInfo] = []
    fileprivate var selectedInfo: SupportLocaleInfo?
    ...
    func updateInfo() {
        ...
        SupportLocaleStore.fetch { [unowned self] result in
            switch result {
                
            case .success(let orderedInfo):
                let data: [String: [SupportLocaleInfo]] = orderedInfo
                /*
                 switch self.type {
                 case .currency:
    ...
    func info(at indexPath: IndexPath) -> SupportLocaleInfo? {
...
extension SupportLocalePickerViewController: UISearchResultsUpdating {
...
extension SupportLocalePickerViewController: UISearchBarDelegate {
...
extension SupportLocalePickerViewController: UITableViewDelegate {
...
extension SupportLocalePickerViewController: UITableViewDataSource {
