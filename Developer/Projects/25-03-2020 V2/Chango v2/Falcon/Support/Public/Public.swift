...
extension UILocalizedIndexedCollation {
  ...
  func partitionObjects(array:[AnyObject], collationStringSelector:Selector) -> ([AnyObject], [String]) {
    ...
    for _ in self.sectionTitles {
		...
		unsortedSections.append(emptyArray) //appending an empty array
    }
...
public let messageStatusSending = ""
...
func dayDifference(from date : Date) -> String {
    ...
    let dates = relativeDateFormatter.string(from: date)
...
func newDateDifference(_ date:Date, numericDates:Bool = false) -> String {
    let calendar = NSCalendar.current
    let unitFlags: Set<Calendar.Component> = [.minute, .hour, .day, .weekOfYear, .month, .year, .second]
    let now = Date()
    let earliest = now < date ? now : date
    ...
    if (components.year! == 0 && components.month! == 0 && components.day! >= 1){
        ...
        return "Today"//"\(components.second!) seconds ago"
        }
...
func daysLeft(_ date:Date, numericDates:Bool = true) -> String {
    let calendar = NSCalendar.current
    let unitFlags: Set<Calendar.Component> = [.minute, .hour, .day, .weekOfYear, .month, .year, .second]
    let now = Date()
    let earliest = now < date ? now : date
    ...
    if (components.year! >= 2) {
        ...
        return "No days"//"\(components.second!) seconds ago"
    } else {
        return "No days"
    }
}
