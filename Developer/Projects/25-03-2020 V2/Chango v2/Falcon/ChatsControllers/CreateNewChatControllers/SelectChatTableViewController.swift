...
class SelectChatTableViewController: UITableViewController {
  ...
  var filteredUsers = [FalconUser]()
  var filteredUsersWithSection = [[FalconUser]]()
  ...
  @objc func setUpCollation() {
    let (arrayContacts, arrayTitles) = collation.partitionObjects(array: self.filteredUsers, collationStringSelector: #selector(getter: FalconUser.name))
    filteredUsersWithSection = arrayContacts as! [[FalconUser]]
  ...
  fileprivate func setupHeaderSectionWithControls() {
    ...
    filteredUsersWithSection.insert([FalconUser](), at: 0)
  }
