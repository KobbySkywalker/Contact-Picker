...
protocol FalconUsersUpdatesDelegate: class {
  func falconUsers(shouldBeUpdatedTo users: [FalconUser])
}
...
class FalconUsersFetcher: NSObject {
    ...
    userHandle[0] = userQuery.queryEqual(toValue: preparedNumber).observe(.value, with: { (snapshot) in
      ...
      if snapshot.exists() {
        ...
        for child in children {
          ...
          if let thumbnailURLString = FalconUser(dictionary: dictionary).thumbnailPhotoURL, let thumbnailURL = URL(string: thumbnailURLString) {
            SDWebImagePrefetcher.shared.prefetchURLs([thumbnailURL])
          }
					if let index = self.users.firstIndex(where: { (user) -> Bool in
            return user.id == FalconUser(dictionary: dictionary).id
          }) {
            self.users[index] = FalconUser(dictionary: dictionary)
          } else {
            self.users.append(FalconUser(dictionary: dictionary))
          }
  ...
  func rearrangeUsers(users: [FalconUser]) -> [FalconUser] { /* Moves Online users to the top  */
    var users = users
  ...
  func sortUsers(users: [FalconUser]) -> [FalconUser] { /* Sort users by last online date  */
