...
class MessageSender: NSObject {
  ...
  fileprivate func sendTextMessage() {
    ...
    print("chat id: \(conversation?.chatID)")
    ...
    print("run")
    let reference = Database.database().reference().child("chats").child("\(groupId)").childByAutoId()
    let referenceId = reference.key
    print("reference: \(referenceId)")
    
    guard let messageUID = reference.key else { return }
    let messageStatus = messageStatusDelivered
    let timestamp = NSNumber(value: Int(Date().timeIntervalSince1970*1000))
	
    
    let defaultData = ["key": messageUID as AnyObject,
                  "groupId": toID as AnyObject,
                  "groupName": groupName as AnyObject,
                  "status": messageStatus as AnyObject,
                  "seen": false as AnyObject,
                  "userId": fromID as AnyObject,
                  "timestamp": timestamp,
                  "message": text as AnyObject]
    
//    let object = ["key": messageUID as AnyObject,
//                  "groupId": toID as AnyObject,
//                  "status": messageStatus as AnyObject,
//                  "seen": false as AnyObject,
//                  "userId": fromID as AnyObject,
//                  "timestamp": timestamp,
//                  "message": text as AnyObject] as AnyObject
//    let myKey: String = reference.key!
//    let defaultData: [String: AnyObject] = [myKey: object, "key": myKey as AnyObject]
    print("update default: \(defaultData)")
    delegate?.update(with: defaultData)
    print("defaut: \(defaultData)")
  ...
  fileprivate func sendPhotoMessage(object: MediaObject) {
    ...
    let defaultData: [String: AnyObject] = ["key": messageUID as AnyObject,
                                            "groupId": toID as AnyObject,
                                            "groupName": groupName as AnyObject,
                                            "status": messageStatus as AnyObject,
                                            "seen": false as AnyObject,
